...
public class SystemsRepository
{
  ...
  public async Task<bool> deleteSystems(int systemId)
  {
    ...
    {
      var client = new HttpClient();
      var request = new HttpRequestMessage(HttpMethod.Delete, "http://localhost:3333/api/system/delete");
      var content = new StringContent("{\r\n    \"systemId\" : " + systemId + "\r\n}", null, "application/json");
      request.Content = content;
      var response = await client.SendAsync(request);
      if (response.IsSuccessStatusCode)
  ...
  public async Task<System> getSystemById(int systemId)
  {
    ...
    {
      var client = new HttpClient();
      var request = new HttpRequestMessage(HttpMethod.Get, "http://localhost:3333/api/system/" + systemId);
      var response = await client.SendAsync(request);
